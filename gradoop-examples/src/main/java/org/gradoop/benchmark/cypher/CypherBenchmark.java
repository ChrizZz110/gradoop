  ...
  public static void main(String[] args) throws Exception {
    ...
    case "hbase":
      ...
      if (USE_PREDICATE_PUSHDOWN) {
        source = ((HBaseDataSource) source).applyVertexPredicate(Predicates.HBase
          .getVertexFilter(QUERY, FIRST_NAME));
        source = ((HBaseDataSource) source).applyEdgePredicate(Predicates.HBase
          .getEdgeFilter(QUERY));
      }
      break;
    case "accumulo":
      ...
      source = new AccumuloDataSource(accumuloEPGMStore, config);
      // Apply predicate
      if (USE_PREDICATE_PUSHDOWN) {
        source = ((AccumuloDataSource) source).applyVertexPredicate(Predicates.Accumulo
          .getVertexFilter(QUERY, FIRST_NAME));
        source = ((AccumuloDataSource) source).applyEdgePredicate(Predicates.Accumulo
          .getEdgeFilter(QUERY));
      }
      break;
    default :
